  ...
  Future<({List<MyListing> listings, bool hasMore, int totalCount})> fetchListings(int page) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // FIX: Convert all problematic fields before parsing
        final fixedJsonData = jsonData.map((item) {
          final fixedItem = Map<String, dynamic>.from(item as Map<String, dynamic>);
          
          // Convert price from number to string if needed
          if (fixedItem.containsKey('price') && fixedItem['price'] is num) {
            fixedItem['price'] = fixedItem['price'].toString();
          }
          
          // Convert other potential problematic fields
          if (fixedItem.containsKey('brand_model_id') && fixedItem['brand_model_id'] is String) {
            fixedItem['brand_model_id'] = int.tryParse(fixedItem['brand_model_id']);
          }
          
          if (fixedItem.containsKey('number_of_passengers') && fixedItem['number_of_passengers'] is String) {
            fixedItem['number_of_passengers'] = int.tryParse(fixedItem['number_of_passengers']);
          }
          
          if (fixedItem.containsKey('warranty') && fixedItem['warranty'] is String) {
            fixedItem['warranty'] = int.tryParse(fixedItem['warranty']);
          }
          
          return fixedItem;
        }).toList();
        
        final listings = fixedJsonData
            .map((item) => MyListing.fromJson(item))
            .toList();
